...
import {
  TECHNICIAN_NAV_SECTIONS,
  isTechnicianNavItemActive,
} from "../../config/technicianNavConfig";

/**
 * Collapsible technician sidebar styled to match the admin and user consoles.
 * Reuses the same Tailwind tokens so visual language is identical across roles.
 */
function TechnicianSidebar({ mobileOpen, onMobileClose }) {
  const location = useLocation();
  const [collapsed, setCollapsed] = useState(false);
  const [openSections, setOpenSections] = useState(() =>
    Object.fromEntries(TECHNICIAN_NAV_SECTIONS.map((s) => [s.id, s.defaultOpen])),
  );
  ...
  return (
    <>
      ...
      <div className={shellClass}>
        <aside className={`${asideClass} ${widthClass}`} aria-label="Technician workspace">
          ...
          <nav className="flex min-h-0 flex-1 flex-col px-2 py-3" aria-label="Technician sections">
            <div className="flex min-h-0 flex-1 flex-col gap-1 overflow-y-auto overflow-x-hidden pb-2">
              {TECHNICIAN_NAV_SECTIONS.map((section) => {
                const SectionIcon = section.icon;
                const isOpen = openSections[section.id] !== false;
                const sectionActive = section.items.some((item) =>
                  isTechnicianNavItemActive(location.pathname, item),
                );
                    ...
                    {isOpen ? (
                      <div className="mt-1 flex flex-col gap-0.5 pl-2">
                        {section.items.map((item) => {
                          const active = isTechnicianNavItemActive(location.pathname, item);
                          ...
                          return (
                            ...
                            >
                              {ItemIcon ? (
                                <ItemIcon className="h-4 w-4 shrink-0" strokeWidth={1.75} aria-hidden />
                              ) : null}
